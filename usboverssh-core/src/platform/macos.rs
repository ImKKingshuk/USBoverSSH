//! macOS-specific USB device enumeration
//!
//! Uses nusb crate for cross-platform USB enumeration.
//! Note: USB/IP server functionality is Linux-only.

use crate::device::{DeviceClass, DeviceInfo, DeviceSpeed};
use crate::error::{Error, Result};

/// Enumerate USB devices on macOS using nusb
...
fn parse_speed(device: &nusb::DeviceInfo) -> DeviceSpeed {
    // nusb provides speed via the Speed type
    match device.speed() {
