...
fn parse_device_from_sysfs(path: &Path, bus_id: &str) -> Option<DeviceInfo> {
    ...
    let read_attr_u8 = |attr: &str| -> Option<u8> {
        read_attr(attr).and_then(|s| s.parse().ok())
    };
...
pub fn check_usbip_available() -> Result<bool> {
    ...
    let module_path = modules_dir.join(&kernel_version).join("kernel/drivers/usb/usbip");
...
pub fn find_vhci_port(speed: DeviceSpeed) -> Result<(String, u32)> {
    let hub_type = if speed.to_usbip_speed() >= 5 { "ss" } else { "hs" };
    
    let platform_path = Path::new(SYSFS_USB_DEVICES).join("platform");
    
    for entry in fs::read_dir(&platform_path).map_err(|e| {
        Error::VhciPortUnavailable(speed)
    })?.flatten() {
        let name = entry.file_name().to_string_lossy().to_string();
...
pub fn vhci_attach(vhci_path: &str, port: u32, socket_fd: i32, devid: u32, speed: u32) -> Result<()> {
